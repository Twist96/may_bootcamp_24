...
function createDefaultProgramRepository(context, initialPrograms = [], initialBindings = {}) {
  ...
  const has = (identifier, clusterFilter = 'current') => {
    ...
    return umi.isPublicKey(resolvedIdentifier) ? programs.some(p => p.publicKey === resolvedIdentifier) : programs.some(p => p.name === resolvedIdentifier);
  };
  const get = (identifier, clusterFilter = 'current') => {
    const cluster = resolveClusterFilter(clusterFilter);
    const programs = all(clusterFilter);
    const resolvedIdentifier = resolveBinding(identifier);
    const program = umi.isPublicKey(resolvedIdentifier) ? programs.find(p => p.publicKey === resolvedIdentifier) : programs.find(p => p.name === resolvedIdentifier);
    if (!program) {
      throw new errors.ProgramNotRecognizedError(resolvedIdentifier, cluster);
    }
    return program;
  };
  ...
  const resolveError = (error, transaction) => {
    ...
    return resolvedError ?? new errors.ProgramErrorNotRecognizedError(program, error);
  };
  ...
  const resolveBinding = (identifier, stack = []) => {
    if (umi.isPublicKey(identifier)) return identifier;
...
exports.createDefaultProgramRepository = createDefaultProgramRepository;
//# sourceMappingURL=createDefaultProgramRepository.cjs.map
